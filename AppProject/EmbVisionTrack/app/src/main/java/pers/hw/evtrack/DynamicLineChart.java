...
public class DynamicLineChart {
    ...
    private int maxPointCnt = 30;
    private long pointIdx;


    //一条曲线
    public DynamicLineChart(LineChart mLineChart, String name, int color) {
    ...
    //多条曲线
    public DynamicLineChart(LineChart mLineChart, List<String> names, List<Integer> colors) {
        this.lineChart = mLineChart;
        leftAxis = lineChart.getAxisLeft();
        rightAxis = lineChart.getAxisRight();
        xAxis = lineChart.getXAxis();
    ...
    private void initLineChart() {
        ...
        lineChart.setBorderColor(Color.parseColor("#236b8e"));
        ...
        xAxis.setValueFormatter(new IAxisValueFormatter() {
            ...
            public String getFormattedValue(float value, AxisBase axis) {
                if (pointIdx < maxPointCnt)
                    return timeList.get((int)value % timeList.size());

                int idx = (int)(value - pointIdx + maxPointCnt);
                if (idx >= 0)
                    return timeList.get(idx % timeList.size());
                return "";
            }
        });
    ...
    public void addEntry(List<Integer> numbers) {
        ...
        if (timeList.size() >= maxPointCnt) {
            timeList.remove(0);
        }
        timeList.add(df.format(System.currentTimeMillis()));
        for (int i = 0; i < numbers.size(); i++) {
            Entry entry;
            int pointCnt = lineDataSets.get(i).getEntryCount();
            if (pointCnt >= maxPointCnt) {
                lineDataSets.get(i).removeFirst();
                //lineData.removeEntry(0,i);
                entry = new Entry(pointIdx, numbers.get(i));
            } else {
                entry = new Entry(pointCnt, numbers.get(i));
